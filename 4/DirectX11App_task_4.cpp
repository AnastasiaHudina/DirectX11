...
ID3D11DepthStencilState* m_pNormalDepthState = nullptr;
...
bool InitWindow(HINSTANCE hInstance, int nCmdShow)
{
    ...
    g_hWnd = CreateWindow(
        L"DirectX11Window",
        L"DirectX 11 - Текстурированный кубик и Skybox",
        WS_OVERLAPPEDWINDOW,
        100, 100,
        rc.right - rc.left,
        rc.bottom - rc.top,
...
bool InitDirectX()
{
    ...
    normalDepthDesc.StencilEnable = false;

    result = m_pDevice->CreateDepthStencilState(&normalDepthDesc, &m_pNormalDepthState);
    if (FAILED(result)) return false;
